...
- **Byte-compiled artefacts**: If the environment writes `.pyc` files, stale or wrongly-targeted bytecode must not cause a legacy interpreter to load an incompatible cached form, nor cause a modern interpreter to pick up shimmed output. (Decision 2026-07-03: the strip loader MUST NOT write a bytecode cache for the coshsh modules it transforms on <3.8 — source is always re-parsed and re-transformed on the legacy interpreter. This guarantees a transformed `.pyc` can never be produced, cached, or later loaded by a modern interpreter, and no stale unshimmed `.pyc` can shadow the transform. CPython's per-version cache tags already separate `cpython-36` from `cpython-312`; disabling caching on the legacy path removes the remaining risk at a negligible one-time parse cost.)
- **Import ordering**: The compatibility layer MUST be active before the first coshsh module is loaded along each of the **three supported entry paths** — the two console scripts (`bin/coshsh-cook`, `bin/coshsh-create-template-tree`) and the test-suite bootstrap (`tests/conftest.py`). A bare `import coshsh` that **bypasses** all three (e.g. an arbitrary third-party script that imports coshsh directly on a legacy interpreter without first importing `coshsh_pycompat`) is **out of scope and unsupported on Python <3.8**: it will fail with the compile-time `SyntaxError`, exactly as it does today. This limitation is inherent — `coshsh/__init__.py` cannot be made to compile on 3.6 without the layer already installed, and `setup.py` is not editable, so no interpreter-wide auto-activation is provided. (Decision 2026-07-03: scope the guarantee to the supported entry paths rather than add a global `.pth`/`sitecustomize` activation.)
...
- **FR-008**: The compatibility layer MUST make runtime imports that reference symbols absent from the legacy standard library resolvable, without editing the sources. The confirmed closed set (validated on the target container, 2026-07-03) is: `typing.Protocol` and `typing.runtime_checkable` (introduced in Python 3.8) and `subprocess.run(capture_output=...)` (introduced in Python 3.7). If a future source change introduces a new module-load-time symbol gap, extending this set is a spec change, not an open-ended runtime discovery.
